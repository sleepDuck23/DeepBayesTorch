...
def perform_attacks(
    ...
    """
    Perform CW attack on a given model, evaluate accuracy vs. loss-balancing, and save results.

    Args:
        data_name (str): Dataset name ('mnist').
        vae_type (str): Model type ('A', 'B', ..., 'G').
        checkpoint_index (int): Index of the checkpoint to load.
        loss-balancing (list): List of epsilon values for CW attack.
    ...
    with open(
        os.path.join(save_dir, f"{data_name}_accuracy_vs_loss_balancing.json"), "w"
    ) as f:
...
def plot_results(json_file, save_dir, data_name, epsilons):
    ...
    for i, attack in enumerate(attack_methods):
        ...
        axes[i].set_xlabel("loss_balancing")
        axes[i].grid(linestyle="--")
        axes[i].legend()

    # Save the plot
    plt.tight_layout()
    filename = f"{data_name}__accuracy_vs_loss_balancing_combined.png"
    plt.savefig(
        os.path.join(
            save_dir,
            filename,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Perform CW attack on a trained VAE."
    )
    ...
    parser.add_argument(
        ...
        help="List of C values for CW attack.",
    )
    parser.add_argument(
        "--batch_size",
        type=int,
    ...
    if args.plot:
        if args.json_file is None:
            ...
            args.json_file = os.path.join(
                args.save_dir, f"{args.data_name}_accuracy_vs_loss_balancing.json"
            )
