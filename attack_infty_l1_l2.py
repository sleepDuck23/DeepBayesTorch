...
def perform_attacks(
    data_name, epsilons, norms, batch_size, save_dir="./results/", device=None
):
    """
    Perform FGSM, PGD, and MIM attacks with various norms, evaluate accuracy, and save results.

    Args:
        data_name (str): Dataset name ('mnist', 'cifar10', 'gtsrb').
        epsilons (list): List of epsilon values for attacks.
        norms (list): List of norms to use for attacks (e.g., ["l1", "l2", "inf"]).
        batch_size (int): Batch size for evaluation.
        save_dir (str): Directory to save results.
        device (torch.device, optional): Device to run the attacks.
    """
    vae_types = ["A", "B", "C", "D", "E", "F", "G"]
    attack_methods = ["FGSM", "PGD", "MIM"]

    accuracies = {vae_type: {norm: {attack: [] for attack in attack_methods} for norm in norms} for vae_type in vae_types}
    
    for vae_type in vae_types:
        encoder, generator = load_model(data_name, vae_type, 0, device)
        encoder.eval()
        ...
        for norm in norms:
            _, eval_fn = construct_optimizer(X_ph, Y_ph, enc, dec, norm, 10, vae_type)
            ...
            for epsilon in epsilons:
                for attack in attack_methods:
                    ...
                    for images, labels in tqdm(
                        test_loader, desc=f"{attack}, epsilon={epsilon}, norm={norm}"
                    ):
                        ...
                        if attack == "FGSM":
                            adv_images = fast_gradient_method(
                                ...
                                images,
                                eps=epsilon,
                                norm={"l1": 1, "l2": 2, "inf": np.inf}[norm],
                                clip_min=0.0,
                                clip_max=1.0,
                                sanity_checks=False,
                            )
                    ...
                    accuracies[vae_type][norm][attack].append(correct / total)

    with open(
        os.path.join(save_dir, f"{data_name}_accuracy_vs_epsilon_with_norms.json"), "w"
    ) as f:
...
def plot_results(json_file, save_dir, data_name, epsilons, norms):
    with open(json_file, "r") as f:
        accuracies = json.load(f)

    vae_types = list(accuracies.keys())
    attack_methods = list(accuracies[vae_types[0]][norms[0]].keys())
    letter_to_title = {
        "A": "GFZ",
        "B": "GFY",
        "C": "GBZ",
    ...
    for norm in norms:
        fig, axes = plt.subplots(len(attack_methods), 1, figsize=(6, 18))
        ...
        for i, attack in enumerate(attack_methods):
            for j, vae_type in enumerate(vae_types):
                ...
                axes[i].plot(
                    epsilons,
                    accuracies[vae_type][norm][attack],
                    marker="o",
                    label=letter_to_title[vae_type],
                    linewidth=2,
                    color=color,
                )
            axes[i].set_title(f"{attack} ({norm.upper()}) victim accuracy")
            axes[i].set_xlabel("Epsilon")
            axes[i].grid(linestyle="--")
            axes[i].legend()

        plt.tight_layout()
        filename = f"{data_name}_accuracy_vs_epsilon_{norm}.png"
        plt.savefig(
            os.path.join(save_dir, filename)
        )
        plt.close()
        print(
            f"Saved accuracy plot for norm={norm} to", os.path.join(save_dir, filename)
        )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Perform adversarial attacks on a trained VAE."
    )
    ...
    parser.add_argument(
        ...
        help="List of epsilon values for attacks.",
    )
    parser.add_argument(
        "--norms",
        type=str,
    ...
    if args.plot:
        if args.json_file is None:
            ...
            args.json_file = os.path.join(
                args.save_dir, f"{args.data_name}_accuracy_vs_epsilon_with_norms.json"
            )
        plot_results(args.json_file, args.save_dir, args.data_name, args.epsilons, args.norms)
